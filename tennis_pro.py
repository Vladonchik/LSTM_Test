...
# 1 pair 1:00 am
player1_details = ('Olivier Rochus', 29.0, 6.0)
player2_details = ('Fabio Fognini', 31.0, 4.0)

# player1_details = ('Fabio Fognini', 31.0, 4.0)
# player2_details = ('Olivier Rochus', 29.0, 6.0)


# 2 pair 2:15 am
# player1_details = ('Kimberly Birrell', 26.0, 6.0)
# player2_details = ('Gabriella Da Silva Fick', 26.0, 7.0)


player1_exists = True if len(df[df['server1'].str.contains(player1_details[0], case=True) == True]) > 0 else False
player2_exists = True if len(df[df['server2'].str.contains(player2_details[0], case=True) == True]) > 0 else False

if player1_exists:
    print('player 1 exists')
if player2_exists:
    print('player 2 exists')
if not player1_exists or not player2_exists:
    print('check player names...')
    exit(1)
...
data['pbp'], data['first_win_points'], data['second_win_points'] = zip(*data['pbp'].apply(hp.convert_to_num))
data['first_win_games'], data['second_win_games'] = zip(*data['score'].apply(hp.get_score))
data['target'] = data['winner'].apply(lambda x: 0 if x == 2 else 1)
data = data.dropna()
...
new_data.rename(index=str, columns = {'server1': 'server2',
                                      'server2' : 'server1',
...
print('scaled point first: {}'.format(FIRST_SCALED_POINTS))
...
if EXECUTE_MODEL:
    model = keras.models.load_model('models/m3_ConcatedData.dhf5',
...
if TRAIN:
    ...
    # p1 = Dense(256, input_dim=X['server1'].shape[0])(p1_input)
    # p1 = LayerNormalization()(p1)
    # p1 = LeakyReLU()(p1)
    # p1 = Dropout(0.5)(p1, training=True)
    #
    # p2 = Dense(256, input_dim=X['server2'].shape[0])(p2_input)
    # p2 = LayerNormalization()(p2)
    # p2 = LeakyReLU()(p2)
    # p2 = Dropout(0.5)(p2, training=True)

    p3 = Dense(256, input_dim=X['scalar_1_win_points'].shape[0])(p3_input)
    p3 = LayerNormalization()(p3)
    p3 = LeakyReLU()(p3)
    p3 = Dropout(0.5)(p3, training=True)

    p4 = Dense(256, input_dim=X['scalar_2_win_points'].shape[0])(p4_input)
    ...
    input_vecs = Concatenate()([player1, player2, p3_input, p4_input, p5_input, p6_input])
    ...
    model = Model(inputs=[p1_input, p2_input, p3_input, p4_input, p5_input, p6_input], outputs=yy)
    ...
    test_inputs = [test_x['server1'], test_x['server2'], test_x['scalar_1_win_points'], test_x['scalar_2_win_points'], test_x['scalar_1_win_games'], test_x['scalar_2_win_games']]
    train_inputs = [train_x['server1'], train_x['server2'], train_x['scalar_1_win_points'], train_x['scalar_2_win_points'], train_x['scalar_1_win_games'], train_x['scalar_2_win_games']]
    ...
    the_model_name = 'models/' + 'm3_ConcatedData' + '.dhf5'
    the_mcp_save = ModelCheckpoint(the_model_name, save_best_only=True, monitor='val_loss', mode='min')

    the_reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.85, patience=4, min_lr=0.00001, verbose=1)
