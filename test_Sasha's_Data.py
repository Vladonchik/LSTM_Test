...
file_name = 'Candle_BTCUSD'
ratio = 'BTC-USD'
dataset = '{}.csv'.format(file_name)
df = pd.read_csv(dataset)  # read in specific file

df = df[(df != 0).all(1)]  # drop 0's

df.rename(columns={"closePrice": '{}_close'.format(ratio), "volume": '{}_volume'.format(ratio)}, inplace=True)

df.set_index("startTimeStamp", inplace=True)  # set time as index so we can join them on this shared time
...
# print(main_df.head(5))  # how did we do??
# exit(1)
